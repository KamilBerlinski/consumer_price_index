...
def run_oil_gas():
    try:
        print("DEBUG: Fetching News data...", file=sys.stderr)
        data = fetch_market_news()
        print(f"DEBUG: Records dowloaded: {len(data)}.", file=sys.stderr)  

        print("DEBUG: Deduplicating and uploading...", file=sys.stderr)

        bigquery_deduplicate_and_upload(data)
        return jsonify({"status": "success", "count": len(data)}), 200     

    except Exception as e:
        print(f"CRITICAL ERROR in main.py:", file=sys.stderr)
        traceback.print_exc(file=sys.stderr)
        return jsonify({"status": "error", "message": str(e)}), 500        
